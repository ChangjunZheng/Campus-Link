...
public record ReplyVo(Long id, int floorNo, String contentHtml, String authorNickname, Instant createdAt) {

    public static ReplyVo from(ReplyItem item) {
        return new ReplyVo(item.id(), item.floorNo(), item.contentHtml(), item.authorNickname(), item.createdAt());
    }
}
