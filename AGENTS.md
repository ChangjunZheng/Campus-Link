...
mvn verify            # 编译 + 单测（合码前必须全绿，当前基线 294 个（CR-066 后 248 → CR-068 的 G9 后 249 → CR-071 个人中心第一轮后 294）；含架构守护测试）
...
- **单测**：`mvn verify` 必须全绿（**当前基线 294 个**（[CR-066](docs/变更日志/变更台账.md#cr-066) 后 208 → 220 → 248，[CR-068](docs/变更日志/变更台账.md) 加 G9 后 249，[CR-071](docs/变更日志/变更台账.md#cr-071) 个人中心第一轮后 **294**——新增 7 个测试类 35 例 + 改动 3 个类 +10 例，逐类归因见[方案 §8](docs/开发/实施方案/CR-071-个人中心第一轮.md)（+45 例 = 新类 35 + 改类 10；改基线数时须同时改「常用命令」与本条两处），含 `ArchitectureGuardTest` 架构守护测试与 `EndpointAuthorizationManagerTest` 路径级鉴权矩阵）；`MarkdownRendererTest` 的 6 个 XSS 回归用例是论坛安全生命线，渲染 / 白名单改动**先补用例再改实现**；
