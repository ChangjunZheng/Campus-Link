...
当前阶段：**阶段四开发 · Sprint 2 进行中**（Sprint 1 已完成 T1~T9 + 账号上下文 DDD 四层化，ADR-012）。**执行依据是 [`docs/development/sprint-2.md`](docs/development/sprint-2.md)（MVP · 论坛最小闭环）**，不是 sprint-1.md。
**2026-09-12 起按最小 MVP 推进**（[CR-022](docs/change-log.md)）：Sprint 2 只做「发帖 / 列表 / 详情 / 楼层回复」四项，明确 **13 项不做**，验收标准 5 条。**原定的 Sprint 2 开工阻断项 A3-9（架构守护测试）/ A3-10（消除 F-1/F-3/F-4，涉及改代码须另开 CR）已被推迟到 MVP 验收后**（偏离登记为 [W-07](docs/tailoring-waivers.md)）——**推迟 ≠ 免除，[W-02](docs/tailoring-waivers.md) 的关闭前置仍是 A3-9**。治理待签项已于 2026-09-11 全部签署（[CR-020](docs/change-log.md)）。
**⚠️ 本 Sprint 开工前必读 sprint-2.md §4 的三条人工自查**——A3-9 缺位期它们**是唯一的防线**：① `module/forum` 的**依赖方向人工核对**（无测试会拦你，写错不会失败）；② 两个受保护端点（`POST /posts`、`POST /posts/{id}/replies`）**必须人工确认 Controller 里真的写了鉴权**（**N-4 敞口**：契约声明 ≠ 运行时强制，漏写会**静默**变成公开接口）；③ 新增 6 个端点后**重生成 OpenAPI 快照**。
...
| `docs/development/sprint-2.md` | **当前执行依据（MVP · 论坛最小闭环，[CR-022](docs/change-log.md)）**。§1 IN/OUT 范围（**13 项不做**，防范围蔓延）+ 5 条验收标准；§2 六个端点；**§4 三条人工自查（A3-9 缺位期唯一的防线，开工前必读）** |
| `docs/development/sprint-2-design.md` | **Sprint 2 实现依据（增量设计）**。§1 已核实事实（**新增迁移 0 个、论坛端点 0 个已存在**）；§2 `module/forum` 四层文件清单 + **§2.1 六条人工分层自检（写代码时逐条打勾）**；§3 六端点契约（**分页口径 `page/size`**，与 §5 旧表的 `pageNum/pageSize` 不同，落码后须同步 §5）；§4 四处专门决策（**楼层号用 `reply_count` 行锁派生**、**两个受保护端点必须手写鉴权 + 各配 1 个 401 单测**、跨上下文昵称走 `account.application` 新增只读方法、契约快照与 §5 同步）；§6 七条取舍与风险 |
| `docs/next-steps.md` | 阻塞项与并行项，"现在该干什么"入口。**外部依赖阻塞**：B1 / B2 学籍名册（B3 根仓库远程托管已闭环）；**原 Sprint 2 开工阻断项 B4 / B5（= 设计门 A3-9 / A3-10）已由 [CR-022](docs/change-log.md) 推迟至 MVP 验收后**——四个阻塞项仍在表内，但**当前均不阻断 Sprint 2**；**B1 / B2 是"无法做"，B4 / B5 是"可以做但先不做"，后者长期不补会从"欠账"变成"范式固化"** |
...
| `docs/reviews/gate-*.md` | 三道已过评审门的纪要（立项 / 需求 / 设计），**批准记录均已签署**（2026-09-11）——但**批准 ≠ 行动项闭环**：A1-1~A1-6、A2-1~A2-8 全部仍开放；设计门 **A3-1 / A3-2 / A3-3 / A3-4 已闭环**，仍开放 **A3-5 ~ A3-11 共 7 项**，其中 **A3-9 / A3-10 的 Sprint 2 开工前置已由 [CR-022](docs/change-log.md) 推迟至 MVP 验收后** |
...
- **受保护端点必须真的校验鉴权（N-4 未闭环，Sprint 2 期间强制执行）**：`SecurityConfig` 仍是 `anyRequest().permitAll()`，`@SecurityRequirement` **只是契约声明**，登录态由各 Controller 手工校验。**新增任何需要登录的端点（本 Sprint 为 `POST /posts`、`POST /posts/{id}/replies`）时，必须真的写校验代码，并补一个「匿名调用 → 401」的单测**——漏写不会报错、不会让任何现有测试失败，会**静默**变成公开接口，而快照上的 `security` 声明反而制造"已受保护"的错觉。这是 [W-07](docs/tailoring-waivers.md) 推迟 A3-9 的直接后果，闭环时机见 `docs/design/api/README.md` §6.2；
