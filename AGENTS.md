...
| [`docs/需求/产品需求文档-*.md`](docs/需求/产品需求文档.md) | ⚠️ **7 份增量 PRD 草案**（[CR-033](docs/变更日志/变更台账.md) / [CR-036](docs/变更日志/变更台账.md)~[CR-039](docs/变更日志/变更台账.md) / [CR-042](docs/变更日志/变更台账.md) / [CR-067](docs/变更日志/变更台账.md)）**全部"待发起人评审、通过前不得开工"**——个人主页 `/u/:id`、头像体系、账号类型与身份体系、**资料编辑与"我的帖子 / 回帖"列表（F-ACC-007）**等能力都压在这些草案上，**按草案动手前先确认其状态** |
...
mvn verify            # 编译 + 单测（合码前必须全绿，当前基线 249 个（CR-066 后 248 + CR-068 的 G9 一例）；含架构守护测试）
...
- **单测**：`mvn verify` 必须全绿（**当前基线 249 个**（[CR-066](docs/变更日志/变更台账.md#cr-066) 后 208 → 220 → 248，[CR-068](docs/变更日志/变更台账.md) 加 G9 后 249），含 `ArchitectureGuardTest` 架构守护测试与 `EndpointAuthorizationManagerTest` 路径级鉴权矩阵）；`MarkdownRendererTest` 的 6 个 XSS 回归用例是论坛安全生命线，渲染 / 白名单改动**先补用例再改实现**；
