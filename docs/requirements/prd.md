...
> 版本号以 [docs/README.md](../README.md) 第 2 节为单一登记处，本文交叉引用不写版本号（手册 4.5）。2026-09-11 的编辑性修订（头部元数据、§4 表头口径、评审状态如实化）需求内容零改动，登记于 [CR-009](../change-log.md)；**同日随 [CR-013](../change-log.md) 补 F-ACC-004 字段规则，内容有实质变更，故升 v1.2**。
>
> **基线归档缺口**：变更记录由 v0.2 直接跳至 v1.1，**v1.0 基线从未归档**（手册 3.2 要求"评审通过后冻结版本"），已登记为[变更台账](../change-log.md) L-1。

> **变更记录**
>
> - **v1.2（2026-09-11）**——补 **F-ACC-004 字段规则**：学号限定 9 位数字、姓名限中文名或外文名、邮箱格式校验，并在 §3.2 增加对应的格式拒绝验收标准。**动因**：v1.1 只规定"学号 + 姓名比对"，未规定字段格式；开发中（[CR-013](../change-log.md)）已按上述规则实现，本次将基线补正，消除"实现与基线不一致"。**注意**：本改为**基线级内容变更**，按手册 4.2 应经变更评审——当前仅有发起人指令、未召开评审，属既存流程偏离（见 [W-01](../tailoring-waivers.md)）。
...
- 业务规则：
  - 一个学号只能注册一个账号；
  - **字段规则（v1.2 补）**：
    - **学号**：`^\d{9}$`——**恰好 9 位数字**（如 `249971346`）。位数不符、含字母/符号一律在提交时拒绝，不进入名册比对；
    - **姓名**：须为**中文名**（2~16 个汉字，可含 `·` 分隔的复姓或少数民族名，如"欧阳修""阿卜杜拉·买买提"）或**外文名**（以拉丁字母起头，可含空格、`-`、`'`、`.`，如"John Smith""O'Brien""Jean-Luc Picard"）；
    - **邮箱**：须为合法邮箱格式（注册与登录均校验）；
    - 以上格式校验失败须返回**字段级提示**（区别于"学籍信息校验未通过"的统一提示——格式错误不泄露名册信息，故可明确说明）。
...
```gherkin
Given 名册中存在学号 249971346 且姓名为"张三"且未被占用
When 用户提交 学号=249971346 姓名=张三
...
Given 用户输入学号 2023001（7 位）/ 24997134a（含字母）/ 24997-1346（含符号）
When 提交学籍核验
Then 拒绝并提示"学号必须为 9 位数字"，且不消耗 IP 核验次数、不触达名册

Given 用户输入姓名 张三123 / 张（单字）/ 李@四 / <script>
When 提交学籍核验
Then 拒绝并提示姓名须为符合规范的中文名或外文名
```
