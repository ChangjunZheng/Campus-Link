...
| 名册旁路开关 `app.roster.bypass` 的生产关闭校验                                                                                                                                                                                                                  | 风险             | 发布负责人       | 上线检查清单 → **已升级为机器强制（[CR-063](../变更日志/变更台账.md#cr-063)，2026-09-18）**：`config/ProductionSafetyGuard` 以 `prod` profile 启动即校验 `bypass=false` + 固定验证码留空 + 三把密钥与库口令全部覆盖开发值，漏配即启动失败；清单项保留作部署核对                                                                            |
