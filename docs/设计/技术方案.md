...
- **风格**：REST，前缀 `/api/v1`；OpenAPI 由 springdoc-openapi 自动生成，部署于 `/api/docs`；**已归档快照** `[接口契约/openapi.json](接口契约/openapi.json)`（来源、再生成命令、实测请求响应示例见 [api/README.md](接口契约/README.md)；首次归档 [CR-019](../变更日志/变更台账.md)，[CR-021](../变更日志/变更台账.md) 修复 N-1/N-2/N-3 后**已再生成**，[CR-022](../变更日志/变更台账.md) 的 Sprint 2 MVP 新增 6 个论坛端点后**再次生成**，[CR-028](../变更日志/变更台账.md) 闭环 N-4 后**再次生成**（**仅 `bearerAuth` 描述据实改写，端点 / schema / `security` 声明一字未变**，即 v1.3 → v1.4 无破坏性变更），现为 **12 端点 / 30 schema**，含错误响应与 `bearerAuth` 声明）——快照只覆盖**已实现**的端点，本节表格含未实现的计划项（下表有"状态"列逐行标注）；**注意快照的来源当前是未提交的工作区而非某个 commit**，见 api/README §2；
- **鉴权（[CR-028](../变更日志/变更台账.md) 按 N-5 据实修正）**：`Authorization: Bearer <jwt>`，7 天滑动续期；**运行时强制点是 `config/JwtAuthenticationFilter` 解析 token** + **Controller 内调用 `common/web/CurrentUser` 统一入口**（`requireId` → 未登录 `401 / 4001`；`requireRole` → 权限不足 `403 / 4002`）。⚠️ **`config/SecurityConfig` 仍是 `anyRequest().permitAll()`，没有路径级拦截**——即"未登录可读、写操作 401"这一**行为**成立，但**不是**由 Spring Security 按 URL 规则实现的（旧表述让其看起来像框架兜底，与实现不符，N-5）。
  **防漏写由机器强制**：`ArchitectureGuardTest` 要求每个 HTTP 映射方法**显式二选一**——标 `@PublicEndpoint`（公开）或标 `@SecurityRequirement`（受保护），且**标了受保护就必须真的调用 `CurrentUser`**；漏写或只声明不调用 → 测试失败。契约侧由 `@SecurityRequirement(name = ApiDocs.BEARER_AUTH)` 表达（**不要加全局 security**，否则公开端点会被错标为需鉴权）。
  **后台接口**额外校验角色：`role in (ops, superadmin)`，经 `CurrentUser.requireRole(authentication, "ROLE_SUPERADMIN")` 实现（名册导入即此模式）。
  **两点框架行为须知**：① **`@Valid` 先于方法体执行**，故"匿名 + 非法请求体"返回 `400 / 1001` 而非 `401`（匿名 + 合法 body 才是 401）；② `1002` / `1003` / `1004` 三类路径级错误由 `GlobalExceptionHandler` 统一产出，**永不进 OpenAPI 快照**（见 [api/README](接口契约/README.md) §6.3）；
...
| `/api/v1/posts/{id}/replies?page=&size=` | GET           | 楼层列表（`floor_no` 升序）                              | ✅ 已实现（本节原表未列，[CR-022](../变更日志/变更台账.md) 新增） |
