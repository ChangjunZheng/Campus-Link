...
| 变更登记 | [CR-019](../../变更日志/变更台账.md#cr-019)（首次归档）· [CR-021](../../变更日志/变更台账.md#cr-021)（修复 N-1 / N-2 / N-3 后**再生成**：契约新增错误响应与 bearerAuth 声明）· [CR-022](../../变更日志/变更台账.md#cr-022)（Sprint 2 MVP 新增 6 个论坛端点后**再生成**：6 → **12** 端点）· [CR-028](../../变更日志/变更台账.md#cr-028)（**N-4 闭环后**再生成：`bearerAuth` 描述据实改写，**端点 / schema 数一字未变**）· [CR-031](../../变更日志/变更台账.md#cr-031)（**路径级鉴权落地后再生成**：`bearerAuth` 描述随框架拦截据实改写，**端点 / schema / `security` 声明仍未变**）· [CR-043](../../变更日志/变更台账.md)（**采纳最佳答案落地后再生成**：新增 `POST /posts/{id}/accept` 与 `AcceptReplyCommand` schema，12 → **13** 端点）· [CR-048](../../变更日志/变更台账.md)（**点赞收藏落地后再生成**：新增 4 端点与 `InteractionResult` schema，13 → **17** 端点）· [CR-049](../../变更日志/变更台账.md)（**站内搜索落地后再生成**：新增公开端点 `GET /posts/search`，17 → **18** 端点，**schema 数未变**）· [CR-050](../../变更日志/变更台账.md)（**通知中心落地后再生成**：新建 `module/notification` 上下文，新增 3 个受保护端点与 7 个 schema，18 → **21** 操作 / 33 → **40** schema；⚠️ 本次 diff **非纯新增**，另有 1 处 springdoc 的 `operationId` 重编号，见 §2）· [CR-051](../../变更日志/变更台账.md)（**时间戳时区口径修复——非再生成**：只改数据源时区参数、不动接口形状，重抓核对 `git diff` **为空**，快照文件零改动；本文新增 §1「时间字段口径」条与 §2 核对行，见 §7 v1.9.2）· [CR-058](../../变更日志/变更台账.md#cr-058)（**热榜定时任务（ADR-006）+ `sort=hot` 落地后再生成**：**契约规模零变化**，仍 8 tag / 21 操作（19 路径）/ 40 schema / 12 受保护操作；唯一变化是 `GET /api/v1/posts` 多一个 `sort` 查询参数（`default: "latest"`）与该操作 `summary` 的一处据实改写，diff **+10 / −1**，见 §7 v1.10）· [CR-060](../../变更日志/变更台账.md#cr-060)（**修复已下架回复仍可被点赞与采纳——非再生成**：只把楼层写用例的读侧换成带 `status` 过滤的`findVisibleById`，**不动 Controller / VO / `ResultCode` / 鉴权声明**，重抓核对逐字节一致、快照文件零改动，见 §7 v1.10.1）· [CR-065](../../变更日志/变更台账.md#cr-065)（**帖子删除 F-FORUM-006 后端落地后再生成**：新增**受保护**端点 `DELETE /api/v1/posts/{id}`，21 → **22** 操作、受保护 12 → **13**；**路径仍 19、schema 仍 40**（挂既有路径、`ApiResponseVoid` 已存在），diff **+85 / −0 纯新增**，见 §7 v1.11） |
...
- **覆盖范围是**已实现的 **22 个端点**（19 条路径，本文口径为"**一个 operation 计一个端点**"，故路径数与端点数本就不同：`/api/v1/posts` 与 `/api/v1/posts/{postId}/replies` 各含 GET + POST，`/api/v1/posts/{id}` 含 GET + **DELETE**（[CR-065](../../变更日志/变更台账.md#cr-065)）），账号 6 + 论坛 13 + **通知 3**，不得读成"全部接口契约已归档"——[技术方案](../技术方案.md) §5 接口表中仍有多行属后续 Sprint 计划项（逐条差异见 §5）。因此技术方案文末检查清单第 3 项"接口契约**完整**"**仍不勾选**；
...
| **本次生成时间** | **2026-09-18**（[CR-065](../../变更日志/变更台账.md#cr-065) 帖子删除端点落地后**再生成**：新增 1 个受保护 operation，21 → **22** 操作，路径 / schema / tag 数均不变，受保护 12 → **13**）。上一次再生成为 2026-09-16 07:56（[CR-058](../../变更日志/变更台账.md#cr-058) 热榜 `sort=hot`：**契约规模零变化**）；[CR-060](../../变更日志/变更台账.md#cr-060) 与 CR-051 同属**非再生成**（重抓逐字节一致，见本节末的核对行）。再上一次规模变化是 2026-09-14 22:46（[CR-050](../../变更日志/变更台账.md) 通知中心：18 → 21 操作、33 → 40 schema、7 → 8 tag、受保护 9 → 12） |
| **代码来源** | ⚠️ **未提交的工作区（CR-065 + CR-064）**——本次快照由本地 `backend/src/main`（含 `PostController#delete` 新映射方法）跑出，**这两条 CR 均尚未提交**，故按本节既有纪律，**提交后须重抓一次核对**（先例：v1.5.1 / v1.6.1 / v1.7.1 / v1.8.1 / v1.9.1 / v1.10.1）。上一份可追溯到提交的快照来源为 **`286739b`**（CR-058 实现代码），并由 **`8b37194`**（[CR-060](../../变更日志/变更台账.md#cr-060)）二次复核为逐字节一致（60251 字节）。方法学备注（沿用）：⚠️ 抓取须用 **`localhost`** 而非 `127.0.0.1`——springdoc 会把 `servers` 回显成请求 Host，那是**抓取方式**差异、不是契约差异（v1.8.1 已记过一次） |
...
| 规模 | **8 个 tag** / **22 个端点**（19 条路径）/ **40 个 schema** / **13 个受保护操作** —— 与 v1.10 相比**只多 1 个 operation**（`DELETE /api/v1/posts/{id}`），路径数不变（挂在既有路径上）、schema 数不变（`ApiResponseVoid` 已由 `accept` 产出）、tag 数不变（仍属 `post`）；文件 **60251 → 62546 字节**（+2295）。tag 清单：`admin-roster` / `auth` / `board` / `favorite` / `notification` / `post` / `reply` / `user` |
| 落盘处理 | 原始响应单行压缩 → 按 2 空格缩进格式化。**键序保持 springdoc 原序，未排序、未删改任何字段**；本次自**临时端口 8099** 抓取（`APP_ROSTER_BYPASS=false`，8088 / 8089 由其他会话占用、按惯例不动），故 `servers` 照 v1.6 / v1.6.1 / v1.7.1 / v1.10 先例**归一回仓库既有值** `[{"url": "http://localhost:8088", "description": "Generated server url"}]`，除此之外未改任何字段 |
| 可复现性 | 格式化方法与历次一致（`json.dumps(ensure_ascii=False, indent=2)` + 尾部换行），`git diff` 仍可作为契约变更的可靠信号 |
| 本次 diff 性质 | ✅ **纯新增**：`git diff --numstat` **85 行新增 / 0 行删除**——删除行为零即"没有任何既有字段被改写"，也**未复现 v1.9 那类 `operationId` 漂移**（新操作自得 `operationId: delete`，既有 `list` / `list_3` 等编号未动）。85 行恰为一个完整 operation 对象：`tags` / `summary` / `operationId` / `parameters`（路径参数 `id`，`int64`）/ **6 个响应**（`200 ApiResponseVoid` 与 `400`·`401`·`403`·`404`·`500` 各带 `ApiError`）/ `security: [bearerAuth]`。⚠️ **`405` / `415` / 路由级 `404`（`1004`）三类仍不在快照里**，与本文 §6.3 一致，不是本次新增的缺口 |
