...
| **代码来源** | ⚠️ **未提交的工作区（CR-066）**——本次快照由本地 `backend/src/main` （含新建的 `AdminModerationController` 两个映射方法）跑出，CR-066 的代码与文档**均未提交**，故按本节既有纪律，**提交后须重抓一次核对**（先例：v1.5.1 / v1.6.1 / v1.7.1 / v1.8.1 / v1.9.1 / v1.10.1）。〔**v1.11 同行原文保留**：其时来源记为"未提交的工作区（CR-065 + CR-064）"，上一份可追溯到提交的快照来源为 `286739b`（CR-058 实现代码）、并由 `8b37194`（CR-060）二次复核为逐字节一致（60251 字节）。〕⚠️ **v1.11 那一轮的「提交后核对」至今未做**——CR-064 / CR-065 其后已随 **`8216843`** 入库并推送 origin/main，但没人自己提交源码重抓比对过，"v1.11 快照 = 已提交代码"这一条**目前是断言、不是实测**。**两条敞口可并一次闭环**：CR-066 提交后自己提交源码重抓，与仓库快照逐字节比对即可同时确认 v1.11 与 v1.12。方法学备注（沿用）：⚠️ 抓取须用 **`localhost`** 而非 `127.0.0.1`——springdoc 会把 `servers` 回显成请求 Host，那是**抓取方式**差异、不是契约差异（v1.8.1 已记过一次） |
