...
- **覆盖范围是**已实现的 **24 个端点**（21 条路径，本文口径为"**一个 operation 计一个端点**"，故路径数与端点数本就不同：`/api/v1/posts` 与 `/api/v1/posts/{postId}/replies` 各含 GET + POST，`/api/v1/posts/{id}` 含 GET + **DELETE**（[CR-065](../../变更日志/变更台账.md#cr-065)），`/api/v1/admin/{posts,replies}/{id}/status` 各 1 个 **PUT**（[CR-066](../../变更日志/变更台账.md#cr-066)）），账号 6 + 论坛 13 + **通知 3** + **处置 2**，〔v1.11 及之前本行为 **22 个端点 / 19 条路径**，+2 来自 CR-066 两个受保护 PUT〕不得读成"全部接口契约已归档"——[技术方案](../技术方案.md) §5 接口表中仍有多行属后续 Sprint 计划项（逐条差异见 §5）。因此技术方案文末检查清单第 3 项"接口契约**完整**"**仍不勾选**；
- **自 [CR-021](../../变更日志/变更台账.md#cr-021) 起，快照也表达错误响应与鉴权**：每个操作都声明了 4xx / 5xx 及错误体 schema `ApiError`，`components.securitySchemes.bearerAuth` 已定义，**13 个**受保护操作带 `security`（`users/me`、`admin/roster/import`、`POST /posts`、`POST /posts/{postId}/replies`、`POST /posts/{id}/accept`、**`DELETE /posts/{id}`（[CR-065](../../变更日志/变更台账.md#cr-065)）**、点赞 ×2、收藏、`favorites/mine`、**通知 ×3**（列表 / 未读数 / 全部已读，[CR-050](../../变更日志/变更台账.md) —— 前三个不属于账号或论坛上下文的受保护端点）。**N-4 的"漏写鉴权静默变公开"缺口已由 [CR-028](../../变更日志/变更台账.md#cr-028) 闭环**（统一入口 `CurrentUser` + `ArchitectureGuardTest` 机器校验）；**路径级拦截已由 [CR-031](../../变更日志/变更台账.md#cr-031) 落地**（`SecurityConfig` 去 `permitAll()`，按端点注解裁决，见 §6.2），另有 4 类事实快照表达不了（§6）——读快照前必须先看 §6；
...
| 规模 | **9 个 tag** / **24 个端点**（21 条路径）/ **41 个 schema** / **15 个受保护操作** —— 与 v1.11（8 tag / 22 端点 / 19 路径 / 40 schema / 13 受保护，文件 62546 字节）相比 **+2 operation / +2 路径 / +1 schema / +1 tag / +2 受保护**，全部来自 [CR-066](../../变更日志/变更台账.md#cr-066) 内容处置：新 tag `admin-moderation`、新 schema `ModerationCommand`、两个新路径各含 1 个 PUT。文件 **62546 → 68470 字节**（+5924）。tag 清单：`admin-moderation` / `admin-roster` / `auth` / `board` / `favorite` / `notification` / `post` / `reply` / `user` |
| 落盘处理 | 原始响应单行压缩 → 按 2 空格缩进格式化。**键序保持 springdoc 原序，未排序、未删改任何字段**；本次自**临时端口 8096** 抓取（`mvn spring-boot:run` 跑工作区源码；`HOT_REFRESH_INTERVAL_MS=20000` 只影响定时任务、不入契约，8088 / 8089 由并发会话占用、按惯例不动），故 `servers` 照 v1.6 / v1.6.1 / v1.7.1 / v1.10 / v1.11 先例**归一回仓库既有值** `[{"url": "http://localhost:8088", "description": "Generated server url"}]`，除此之外未改任何字段。⚠️ **抓取一律用 `localhost`**（本次即用 `http://localhost:8096/api/docs`）——springdoc 把 `servers` 回显成请求 Host，用 `127.0.0.1` 抓会得到 `http://127.0.0.1:8096`，那是抓取方式差异、归一时须一并处理 |
| 可复现性 | 格式化方法与历次一致（`json.dumps(ensure_ascii=False, indent=2)` + 尾部换行），`git diff` 仍可作为契约变更的可靠信号 |
| 本次 diff 性质 | ✅ **纯新增，但删除行不为零——这一条必须逐行判定**：`git diff --numstat` **+232 / −16**。⚠️ 那 16 行删除**不是任何既有字段被改写**，而是 `ApiResponseVoid` 这一个 schema 块**整体换位**（新增的两个 PUT 也引用它，springdoc 的 schema 顺序随之重排），机器核对方式：把 HEAD 版与工作区版分别 `json.load` 后比对——**40 个既有 schema 逐一 `sort_keys` 序列化后 byte-identical、既有 operation 逐个比对零变化**，`components.schemas` 的键集合差只多 `ModerationCommand`。〔**教训**：v1.11 那轮写的是"0 行删除即纯新增"，本轮**同一条判定不能照抄**——一旦 springdoc 重排 schema，"−0"这个信号就失效，改判据为"删除行必须逐行归因 + 结构化比对"〕新增的两个 operation 各含 `tags` / `summary` / `operationId` / `parameters`（路径参数 `id`，`int64`）/ **6 个响应**（`200 ApiResponseVoid` 与 `400`·`401`·`403`·`404`·`500` 各带 `ApiError`）/ `security: [bearerAuth]`；新 schema `ModerationCommand` = `{status: string(pattern PUBLISHED|REMOVED, minLength 1, required), reason: string(maxLength 200)}`。⚠️ **`405` / `415` / 路由级 `404`（`1004`）三类仍不在快照里**，与本文 §6.3 一致，不是本次新增的缺口 |
...
### 3.2 论坛上下文（`module/forum`，13 个：Sprint 2 MVP 6 个 + [CR-043](../../变更日志/变更台账.md) 采纳 1 个 + [CR-048](../../变更日志/变更台账.md) 点赞收藏 4 个 + [CR-049](../../变更日志/变更台账.md) 站内搜索 1 个 + [CR-065](../../变更日志/变更台账.md#cr-065) 帖子删除 1 个）
