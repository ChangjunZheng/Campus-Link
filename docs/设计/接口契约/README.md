...
| **本次生成时间** | **快照文件自 2026-09-16 07:56 起未再改动**（最近一次**再生成**为该时点的 [CR-058](../../变更日志/变更台账.md#cr-058)；其后的 [CR-060](../../变更日志/变更台账.md#cr-060) 与 CR-051 同属**非再生成**——2026-09-18 重抓为逐字节一致，见本节末两行）。原记录：**2026-09-16 07:56**（[CR-058](../../变更日志/变更台账.md#cr-058) 热榜定时任务 + `sort=hot` 落地后**再生成**：**契约规模零变化**——仍 8 tag / 21 操作（19 路径）/ 40 schema / 12 受保护操作；唯一变化是 `GET /api/v1/posts` **多一个 `sort` 查询参数**（`required: false`、`default: "latest"`）与该操作 `summary` 的一处改写。上一次为 2026-09-14 22:46（[CR-050](../../变更日志/变更台账.md) 通知中心：18 → 21 操作、33 → 40 schema、7 → 8 tag、受保护 9 → 12） |
| **代码来源** | **提交 `286739b`（[CR-058](../../变更日志/变更台账.md#cr-058) 的实现代码随该提交入库）**，并由 **`8b37194`（[CR-060](../../变更日志/变更台账.md#cr-060)）二次复核为逐字节一致**——原标"⚠️ 未提交的工作区（CR-058）"、要求提交后补做核对的敞口**已于 2026-09-18 关闭**（方法学沿用 v1.5.1 / v1.6.1 / v1.7.1 / v1.8.1 / v1.9.1 先例）。上一份可追溯到提交的快照为 **`70c6ca9`**（CR-050；2026-09-15 自 `http://localhost:8088/api/docs` 重抓后与仓库快照**逐字节一致**，59977 字节）。方法学备注（沿用）：⚠️ 抓取须用 **`localhost`** 而非 `127.0.0.1`——springdoc 会把 `servers` 回显成请求 Host，那是**抓取方式**差异、不是契约差异（v1.8.1 已记过一次） |
...
| 规模 | **8 个 tag** / **21 个端点**（19 条路径）/ **40 个 schema** —— **与 v1.9 完全相同**（CR-058 未增减任何端点或模型，只给既有端点加了一个查询参数）；文件 **59977 → 60251 字节**（+274）。tag 清单：`admin-roster` / `auth` / `board` / `favorite` / `notification` / `post` / `reply` / `user` |
| 落盘处理 | 原始响应单行压缩 → 按 2 空格缩进格式化。**键序保持 springdoc 原序，未排序、未删改任何字段**；本次自**临时端口 8095** 抓取（8088 被 CR-058 之前的旧实例占用、按发起人选择不动它），故 `servers` 照 v1.6 / v1.6.1 / v1.7.1 先例**归一回仓库既有值** `[{"url": "http://localhost:8088", "description": "Generated server url"}]`，除此之外未改任何字段 |
| 可复现性 | 格式化方法与历次一致（`json.dumps(ensure_ascii=False, indent=2)` + 尾部换行），`git diff` 仍可作为契约变更的可靠信号 |
| 本次 diff 性质 | ✅ **纯新增 + 一处 summary 改写**：`git diff --numstat` **10 行新增 / 1 行删除**。**唯一的删除行**即 `GET /api/v1/posts` 的 `summary`（"…boardCode 缺省为全站最新，排序固定 created_at DESC" → "…sort=latest（缺省，created_at DESC）或 hot（hot_score DESC，热榜），非法值 400"）——属**据实改写、非契约破坏**（该端点的路径 / 方法 / 出参 / `security` 全部未变，仍是 `@PublicEndpoint` 公开读）；**10 行新增**恰为 `sort` 查询参数对象（`in: query` / `required: false` / `schema.type: string` / `default: "latest"`），位置插在 `boardCode` 与 `page` 之间，无其他既有字段被改动。⚠️ **本次未复现 v1.9 的 `operationId` 漂移**——只加参数不加方法，`GET /api/v1/posts` 的 `operationId` 仍为 `list`、`GET /api/v1/boards` 仍为 `list_3`。**纪律沿用**：`git diff` 出现删除行即须逐行看清并给出判定，**不能默认"只是多了个参数"** |
