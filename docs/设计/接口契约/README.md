...
| **本次生成时间** | **2026-09-16 07:56**（[CR-058](../../变更日志/变更台账.md#cr-058) 热榜定时任务 + `sort=hot` 落地后**再生成**：**契约规模零变化**——仍 8 tag / 21 操作（19 路径）/ 40 schema / 12 受保护操作；唯一变化是 `GET /api/v1/posts` **多一个 `sort` 查询参数**（`required: false`、`default: "latest"`）与该操作 `summary` 的一处改写。上一次为 2026-09-14 22:46（[CR-050](../../变更日志/变更台账.md) 通知中心：18 → 21 操作、33 → 40 schema、7 → 8 tag、受保护 9 → 12） |
| **代码来源** | ⚠️ **未提交的工作区（[CR-058](../../变更日志/变更台账.md#cr-058)）**——本次快照抓自 CR-058 的实现代码，该 CR **尚未提交**，故提交后须按 v1.5.1 / v1.6.1 / v1.7.1 / v1.8.1 / v1.9.1 同例补做**「提交后核对」并回填提交号**。上一份可追溯到提交的快照为 **`70c6ca9`**（CR-050；2026-09-15 自 `http://localhost:8088/api/docs` 重抓后与仓库快照**逐字节一致**，59977 字节）。方法学备注（沿用）：⚠️ 抓取须用 **`localhost`** 而非 `127.0.0.1`——springdoc 会把 `servers` 回显成请求 Host，那是**抓取方式**差异、不是契约差异（v1.8.1 已记过一次） |
