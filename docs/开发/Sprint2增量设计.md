...
| 维护人 | 技术负责人（发起人兼任） |
| 关联阶段 | 开发（阶段四） |
| 上游依据 | [Sprint 2 计划（MVP）](Sprint2计划.md) · [技术方案 §2.4 / §5](../设计/技术方案.md) · [PRD](../需求/产品需求文档.md)（F-FORUM-001~004）· [API 契约快照](../设计/接口契约/README.md) |
| 下游记录 | [CR-022](../变更日志/变更台账.md) 的后续实施记录（落码与验收结果）· [Sprint 2 计划](Sprint2计划.md) §6 验收记录 |
| 最后更新 | 2026-09-12 |
...
按 [sprint-2.md](Sprint2计划.md) §3 原计划，M1（增量 PRD）与 M2（增量设计）是两份产出。**实际只出本文一份，这是有意的决定**，理由：

- **需求侧没有新东西可写**。F-FORUM-001~004 已在 [PRD](../需求/产品需求文档.md) 中定义并被评审过，本 Sprint 做的是**它们的子集**（去掉标签、热门、引用回复）。再写一份 PRD 只是把"做哪几条"抄一遍，属于 [W-07](../流程偏离记录.md) 所记"流程复杂度已远超代码量"的成因本身；
- **需求侧的裁剪已在 [sprint-2.md](Sprint2计划.md) §1 写清**——IN 4 项、OUT 13 项、验收标准 5 条，该文件即本 Sprint 的需求基线；
- 本文只写**代码里看不出来的决策**：分层归属、契约形状、并发与一致性、任务分解。能从既有代码抄的部分不复述。

> **不代表需求评审被绕过**：[PRD](../需求/产品需求文档.md) 仍是需求基线且为"有条件通过 · 行动项未闭环"，[W-01](../流程偏离记录.md)（单人评审）偏离依旧存在。本文只是**不为本 Sprint 单独再造一份需求文档**。
...
> [ADR-012](../设计/技术方案.md) 要求四层单向依赖：`web` / `infrastructure` → `application` → `domain`；**端口定义在 domain、实现在 infrastructure（DIP）**；跨上下文只调对方 `application`。
> ⚠️ **A3-9 未落地，这套规则没有测试保护（[W-07](../流程偏离记录.md)）**——下面是逐层对照，写完请按 [sprint-2.md](Sprint2计划.md) §4 第 1 条**逐行核对包名与 import**。
...
> ⚠️ 这是本设计**唯一触碰 `account` 模块**的地方，改动仅"新增一个只读方法"，**不改任何既有行为**，故不另行开 CR，登记在本文与 [CR-022](../变更日志/变更台账.md) 的后续实施记录中。

### 4.4 契约快照与文末检查清单

- 落码后按 [api/README](../设计/接口契约/README.md) §2 重生成 `openapi.json`（6 → **12 个端点**）；
...
## 5. 本 Sprint 不做（与 [sprint-2.md](Sprint2计划.md) §1 的 OUT 一致，此处只列**容易被顺手做掉**的）
...
| D-4 | 🔴 **N-4** | 两个受保护端点的鉴权靠**人工手写**，漏写不会失败、会静默变公开接口。补偿：§4.2 的两个单测 + [sprint-2.md](Sprint2计划.md) §4 第 2 条自查 |
...
## 7. 任务分解（照 [sprint-2.md](Sprint2计划.md) §3，落到文件）
...
| M6 | 契约同步 + 验收 | 快照再生成、技术方案 §5、5 条验收标准实测 | M4 | [sprint-2.md](Sprint2计划.md) §1 的 A1~A5 |
...
| # | 偏差 | 实际落码 | 性质与处置 |
|---|------|---------|-----------|
| **P-1** | §3.4 写 `isAccepted` | 实际字段名为 **`accepted`**（Java record 组件名 `accepted` → Jackson 序列化即 `accepted`，无 `is` 前缀） | **文档错误，已修正 §3.4 正文**（本版唯一改动正文的偏差）。前端 `forum.ts` 已按 `accepted` 对接；[api/README](../设计/接口契约/README.md) §3.2 已加显式警示 |
| **P-2** | §3.3 / §3.4 的路径参数写 `{id}` | `ReplyController` 用类级 `@RequestMapping("/api/v1/posts/{postId}/replies")`，故回复两个端点的路径参数名为 **`{postId}`** | **非错误、不改**：两个 Controller 各自命名，OpenAPI 里如实呈现为 `/posts/{id}` 与 `/posts/{postId}/replies`。[api/README](../设计/接口契约/README.md) §3.2 已加说明，避免读者误读为两个资源 |
...
- **v1.1（2026-09-12）——M2~M6 落码后的同步，范围与取舍未变**：① **修正 §3.4 的 `isAccepted` → `accepted`**（P-1，文档错误）；② 新增 **§8 落码偏差记录**（8 条：P-1 文档错误 / P-2 路径参数名 / P-3~P-5 文件清单外的合理补充 / P-6 前端文件说明 / P-7 分页归一化位置 / P-8 方法签名），并附 **§2.1 六条自检的实测证据**（grep 级结论：domain 无框架 import、Mapper 在 `.mapper` 包内、web 无持久化类型、零 `account.infrastructure` import）；③ **§1~§7 的范围、契约形状（除 P-1）、取舍与风险一律未改**——本版只做"把落码事实登记清楚"，不借机改写设计原文；④ 任务 M1~M6 的实际完成情况见 [sprint-2.md](Sprint2计划.md) 与 [CR-022](../变更日志/变更台账.md) 的后续实施记录。
- v1.0（2026-09-12）——创建（[CR-022](../变更日志/变更台账.md) 的后续实施产物）。**只出一份增量文档、不出单独增量 PRD**（理由见 §0，直接回应 [W-07](../流程偏离记录.md) 所记"流程复杂度远超代码量"）；§1 核实并写明**新增迁移 0 个、论坛端点 0 个已存在**；§2 给出 `module/forum` 四层文件清单与 **§2.1 六条人工自检**（A3-9 缺位期的替代）；§3 给出 6 个端点的请求/响应形状与**统一分页口径 `page/size`**；§4 交代四处必须专门的决策——**楼层号与 `reply_count` 的一致性**（行锁派生、不加唯一约束）、**N-4 下两个受保护端点必须手写鉴权并各配一个 401 单测**、**作者昵称走 `account.application` 新增只读方法而非直连对方数据表**、**契约快照与技术方案 §5 同步**；§6 如实登记 7 条取舍与风险（含 🔴 N-4 与 ⚠️ A3-9 缺位）；§7 把任务落到具体文件。
