...
- **P1 UI 稿 → [W-03](../tailoring-waivers.md) 补偿措施**（= 设计门 **A3-6**，**仍开放**）：**2026-09-11 发起人签署"接受"裁剪**（[CR-020](../change-log.md)），确认不产出交互稿与视觉稿、直接用 Element Plus 拼页面；替代交付物为①组件与交互规范约定、②页面截图归档至 `docs/design/ui/`、③提测前逐页走查——签署时**三条均未执行**。**2026-09-12 更新（[CR-023](../change-log.md)）**：**② 的产物已齐备**——目录已建立、[ui-guideline](../design/ui/ui-guideline.md) 已写**页面清单 + 组件与交互规范约定**、**12 张截图已归档**（⚠️ **程序化采集**：无头 Chrome + CDP，单内核、单机、开发库数据——**不等于人工逐页采集**）；**① 的"写入手册 5.1 可裁剪项"那一半与 ③ 仍未执行**，故 **A3-6 仍开放**。详见 [tailoring-waivers](../tailoring-waivers.md) W-03 行。
...
- **v1.13（2026-09-12）**——**随 [CR-023](../change-log.md)（前端样式体系重构）与 12 张页面截图归档做状态回填，Sprint 1 的任务范围与完成情况零改动**：① "阻塞与依赖" **P1 UI 稿条据实更新**——[W-03](../tailoring-waivers.md) 补偿② 的产物**已齐备**（`docs/design/ui/` 已建立、[ui-guideline](../design/ui/ui-guideline.md) 已写页面清单 + 组件与交互规范约定、**12 张截图已归档**，⚠️ **程序化采集**：无头 Chrome + CDP，单内核、单机、开发库数据，**不等于人工逐页采集**）；补偿① 的"写入手册 5.1 可裁剪项"那一半与补偿③（提测前逐页走查）**仍未执行**，故 **A3-6 仍开放**；② 补记补充事实：Sprint 2 交付后的单测基线为 **87/87**（Sprint 2 新增 25 个论坛用例），但**不改变**"覆盖率不可测""静态扫描未接入"两条未满足结论——**提测准入门仍是 7 项中 4 项未满足**。**B1 / B2 ~ B5 各条结论与本文其余内容未变。**
