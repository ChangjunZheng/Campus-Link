...
当前处于**阶段四开发 · Sprint 2 进行中**（Sprint 1 已完成 T1~T9）——**2026-09-12 起按最小 MVP 推进**（[CR-022](docs/change-log.md)）：**Sprint 2 只做「发帖 / 列表 / 详情 / 楼层回复」四项，明确 13 项不做**，执行依据见 [Sprint 2 计划（MVP）](docs/development/sprint-2.md)；**原定的 Sprint 2 开工阻断项 A3-9（架构守护测试）/ A3-10（消除 F-1/F-3/F-4）已推迟到 MVP 验收后**（偏离登记为 [W-07](docs/tailoring-waivers.md)）——**推迟 ≠ 免除**，[W-02](docs/tailoring-waivers.md) 的关闭前置仍是 A3-9，且 A3-9 缺位期**四层依赖方向无机器强制**、**N-4 敞口被放大**（本 Sprint 新增 2 个受保护端点，漏写鉴权会静默成为公开接口），开工前必读 sprint-2.md §4 的三条人工自查。

三道已过的评审门均为**有条件通过**且行动项未闭环（A1-1~A1-6、A2-1~A2-8、A3-5~A3-11），外部依赖 **B1 / B2 学籍名册仍开放**（B3 仓库远程已于 2026-09-11 闭环）。**治理待签项已于 2026-09-11 全部签署**（[CR-020](docs/change-log.md)）：W-01~W-05 接受让步放行、W-06 接受并关闭、三份门纪要批准行逐角色签署、技术方案 v0.3/v0.4 签署「②有条件追认」使设计门 A3-2 闭环——**但签署只批准结论，未闭环任何行动项**。**契约缺口 N-1 / N-2 / N-3 已于 2026-09-12 修复**（[CR-021](docs/change-log.md)：错误响应与 `bearerAuth` 写入 OpenAPI 契约、畸形请求体由 `500 / 9999` 改回 `400 / 1001` 并降日志档位，真机复测通过、快照已再生成），同时**新登记 N-4**——契约声明了鉴权但 `SecurityConfig` 仍 `permitAll()`，**声明 ≠ 强制**，归口 A3-9。详见[文档中心](docs/README.md)第 1 节与[裁剪与让步放行记录](docs/tailoring-waivers.md)。
...
- [**下一步行动清单**](docs/next-steps.md) — 阻塞项（B1/B2 名册 + **B4/B5 已推迟至 MVP 验收后**）/ 并行项 / Sprint 拆解，"现在该干什么"
- [**Sprint 2 计划（MVP）**](docs/development/sprint-2.md) — **当前执行依据**：发帖 / 列表 / 详情 / 楼层回复四项 + 13 项不做 + 5 条验收标准 + §4 三条人工自查
- [**变更台账**](docs/change-log.md) — 全部基线级变更与影响评估（CR-001~**CR-022**）
- [**裁剪与让步放行记录**](docs/tailoring-waivers.md) — **7 项流程偏离**（W-07 为 2026-09-12 新增：最小 MVP 优先，推迟 A3-9 / A3-10），**已于 2026-09-11 / 09-12 由发起人签署**（W-06 关闭；**W-02 / W-05 签署但不得关闭**）
...
- 设计：[技术方案](docs/design/tech-design.md)（ADR-001~012）· [API 契约快照](docs/design/api/README.md)（`openapi.json` + 主链路与错误响应实测 + 仍开放的 N-4/N-5/N-6）
- 开发：[Sprint 1 计划](docs/development/sprint-1.md)（已执行）· [**Sprint 2 计划（MVP）**](docs/development/sprint-2.md)（**进行中**）· [**Sprint 2 增量设计**](docs/development/sprint-2-design.md)（**实现依据**：四层文件清单 + `module/forum` 分层自检 + 六端点契约）
...
- [x] 阶段三 · 设计 — 🟡 **有条件通过**；技术方案 v0.1 经评审会，**v0.2 至今无任何形式评审**，**v0.3 / v0.4（含已落地的 DDD 重构）已于 2026-09-11 签署「②有条件追认」**（发起人单人书面确认，**非评审会**，[A3-2 闭环](docs/reviews/retro-review-design-v03-v04.md)）；**UI 交互稿 / 视觉稿经 [W-03](docs/tailoring-waivers.md) 签署裁剪、不再产出**，替代的三条补偿措施（组件规范约定 / 截图归档 / 逐页走查）**均未执行**，故 A3-6 仍开放；**A3-4 已闭环**——[API 契约快照](docs/design/api/README.md) 于 2026-09-11 归档（[CR-019](docs/change-log.md)）、2026-09-12 随 [CR-021](docs/change-log.md) 修复 **N-1 / N-2 / N-3** 后再生成，但**出口标准第 2 项（数据模型与接口契约完整）仍为 ❌**（A3-5 ER 图未做、快照仅覆盖已实现的 6 个端点、**N-4「声明 bearerAuth ≠ 运行时强制」未闭环**）
- [ ] 阶段四 · 开发 — **Sprint 2 进行中**（Sprint 1 已完成 T1~T9）；提测准入门 **7 项出口标准中 4 项未满足**（① 覆盖率不可测，无 jacoco；② 静态扫描未接入；③ CI 因缺扫描环节不能判"全绿"；④ Code Review 未闭环——见 [W-05](docs/tailoring-waivers.md)）；**2026-09-12 改按最小 MVP 推进**（[CR-022](docs/change-log.md)）：Sprint 2 范围锁定「发帖 / 列表 / 详情 / 楼层回复」四项 + **13 项不做**，验收标准 5 条，**原开工阻断项 A3-9（架构守护测试）+ A3-10（消除 F-1/F-3/F-4）推迟至 MVP 验收后**（偏离登记为 [W-07](docs/tailoring-waivers.md)，**推迟 ≠ 免除**）。原"A3-10 与契约缺口 N-1/N-2/N-3 合并为一个 CR、约 1.5~2 天"的建议**已被超越**——三项已由 [CR-021](docs/change-log.md) 单独做完，**A3-10 余量降为约 0.5 天**；新登记的 **N-4** 归入 A3-9、**N-5 / N-6** 建议并入 A3-10，两项合计约 **1 天**，**在 MVP 验收后补做**
- [ ] 阶段五 · 测试
...
四项决议的完整影响评估见[变更台账](docs/change-log.md)。后续另有 CR-006（开发节奏 6 → 8 周）、CR-007（JDK 21 + Boot 4.1）、CR-008（账号上下文 DDD 重构）、CR-009（治理记录补录）、CR-010（三仓库 → 根目录 monorepo）、CR-011（版本控制闭环）、CR-012（开发期环境改本机 MySQL/Redis、后端端口 8088）、CR-013（字段校验收紧 + dev 便利开关）、CR-014（Flyway 管理 DDL + 数据脚本规范）、CR-015（Redis 键命名空间前缀）、CR-016（唯一键冲突翻译为业务错误）、CR-017（修正技术方案 D-1~D-7）、CR-018（归档 v0.3/v0.4 追认材料）、CR-019（归档 OpenAPI 快照）、**CR-020（发起人签署全部治理待签项）**、**CR-021（修复契约缺口 N-1/N-2/N-3，再生成快照）**、**CR-022（改按最小 MVP 推进，推迟 A3-9 / A3-10）**。
