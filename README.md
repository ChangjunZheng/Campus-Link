...
- [**下一步行动清单**](docs/next-steps.md) — 阻塞项（B1/B2 名册 + **B4/B5 Sprint 2 阻断项**）/ 并行项 / Sprint 拆解，"现在该干什么"
- [**变更台账**](docs/change-log.md) — 全部基线级变更与影响评估（CR-001~CR-021）
- [**裁剪与让步放行记录**](docs/tailoring-waivers.md) — 6 项流程偏离，**已于 2026-09-11 由发起人签署**（W-06 关闭；**W-02 / W-05 签署但不得关闭**）
...
- [ ] 阶段四 · 开发 — **Sprint 1 完成（T1~T9）**；提测准入门 **7 项出口标准中 4 项未满足**（① 覆盖率不可测，无 jacoco；② 静态扫描未接入；③ CI 因缺扫描环节不能判"全绿"；④ Code Review 未闭环——见 [W-05](docs/tailoring-waivers.md)）；**Sprint 2 开工阻断项：A3-9 架构守护测试 + A3-10 消除 F-1/F-3/F-4**（后者涉及改代码须另开 CR）。原"A3-10 与契约缺口 N-1/N-2/N-3 合并为一个 CR、约 1.5~2 天"的建议**已被超越**——三项已由 [CR-021](docs/change-log.md) 单独做完，**A3-10 余量降为约 0.5 天**；新登记的 **N-4** 归入 A3-9、**N-5 / N-6** 建议并入 A3-10，两项合计约 **1 天**
- [ ] 阶段五 · 测试
